...
把这一篇的东西拼起来，`CpuSlimeSolver` 现在长这样——质点状态、子步循环、积分、反推速度。这是求解器的地基，后面每篇往 `Step()` 里插一行约束调用：

```csharp
using UnityEngine;

namespace GraphicsLearning.SoftBody
{
    internal sealed class CpuSlimeSolver
    {
        private const float Epsilon = 0.0000001f;

        private readonly Vector3[] _restPositions;      // 静止参考位置
        private readonly Vector3[] _positions;          // 当前位置
        private readonly Vector3[] _previousPositions;  // 子步开始时的位置
        private readonly Vector3[] _velocities;          // 速度
        private readonly Vector3[] _incomingVelocities; // 入射速度（碰撞回弹用，见 05）

        public CpuSlimeSolver(SlimeTopology topology)
        {
            _restPositions = (Vector3[])topology.RestPositions.Clone();
            _positions = (Vector3[])topology.RestPositions.Clone();
            _previousPositions = new Vector3[_positions.Length];
            _velocities = new Vector3[_positions.Length];
            _incomingVelocities = new Vector3[_positions.Length];
        }

        // 帧率无关插值率：后面每个位置投影都用它控制「一帧拉多少」
        private static float ToProjectionFraction(float rate, float deltaTime)
        ...
        public void Simulate(float deltaTime, int substeps, SoftBodyStepParameters parameters)
        {
            ...
            for (int i = 0; i < substeps; i++)
                Step(substepTime, substeps, parameters);
        }
        ...
        private void BeginStep()
        {
            for (int i = 0; i < _positions.Length; i++)
            {
                _previousPositions[i] = _positions[i];   // 快照，末尾算速度用
                _incomingVelocities[i] = _velocities[i]; // 入射速度，碰撞回弹用
            }
        }
        ...
        {
            for (int i = 0; i < _velocities.Length; i++)
                _velocities[i] += gravity * deltaTime;   // v += a·dt
        }
...
> [!note] 这是「教学精简版」，抓主干
> 真实工程里 `Step()` 还夹着压缩/驱动加速度、接触法线的速度响应等细节（后续篇章补上）。这里剥掉旁支，只留「状态 + 子步 + 积分 + 反推速度」的骨架，让你看清主线。每往后一篇，我们就在这个骨架上加一层，到 06 你会拿到完整版。
