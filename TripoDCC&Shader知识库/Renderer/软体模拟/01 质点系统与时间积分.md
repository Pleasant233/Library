...
> 承接 [[00 什么是软体模拟]]。所有约束都建立在「一堆点在重力下正确运动」之上——这一篇打地基。
...
先给速度施加外力（重力等），再用**更新后的速度**推进位置——这就是半隐式欧拉：
...
> [!tip] 为什么是 `1 - exp(-k·dt)`
> 它是「指数趋近」的离散形式：不管 `dt` 多大，1 秒后剩余的偏差恒为 `exp(-k)`，与帧率无关。高帧率不会过冲、低帧率不会欠响应。
