...
> 承接 [[05 碰撞与接触]]。史莱姆现在能落地摊开了。但把它压得很扁、或对折时，你会看到表面自己穿过自己——像张没有厚度的纸。这一篇加上「自碰撞」补上厚度感；而它带来的性能问题（每个点都要和附近的点比），会引出图形/物理里最常用的一个加速结构：**空间哈希**。
...
```csharp
private readonly Dictionary<Vector3Int, int> _selfCollisionCells;  // 格子 → 链表头
private readonly int[] _selfCollisionNext;                          // 每点 → 同格下一个点

private void SolveSelfCollision(float radiusScale, float exclusionScale)
{
    float minimumDistance = _particleRadius * 2f * radiusScale;
    if (minimumDistance <= Epsilon) return;

    float minimumDistanceSquared = minimumDistance * minimumDistance;
    float exclusionDistance = _particleRadius * exclusionScale;
    ...
    for (int i = 0; i < _surfaceParticleIndices.Length; i++)
    {
        ...
        for (int x = -1; x <= 1; x++)
        for (int y = -1; y <= 1; y++)
        for (int z = -1; z <= 1; z++)   // 3×3×3 邻域
        {
            var neighborCell = new Vector3Int(cell.x + x, cell.y + y, cell.z + z);
            if (!_selfCollisionCells.TryGetValue(neighborCell, out int a)) continue;
            while (a >= 0)              // 沿链表遍历该格已插入的点
            {
                SolveSelfCollisionPair(a, b, minimumDistance, minimumDistanceSquared, exclusionDistanceSquared);
                a = _selfCollisionNext[a];
            }
        }
        // 先查询后插入 → 每对只算一次
        _selfCollisionNext[b] = _selfCollisionCells.TryGetValue(cell, out int head) ? head : -1;
...
private void SolveSelfCollisionPair(int a, int b, float minimumDistance,
                                    float minimumDistanceSquared, float exclusionDistanceSquared)
{
    // 静止时就很近的拓扑邻居跳过，否则表面被自己炸开
    if ((_restPositions[b] - _restPositions[a]).sqrMagnitude <= exclusionDistanceSquared) return;

    Vector3 delta = _positions[b] - _positions[a];
    float sqrDistance = delta.sqrMagnitude;
    if (sqrDistance >= minimumDistanceSquared) return;   // 够远

    float distance = sqrDistance <= Epsilon ? 0f : Mathf.Sqrt(sqrDistance);
    Vector3 direction = distance > Epsilon ? delta / distance : Vector3.right;
    Vector3 correction = direction * ((minimumDistance - distance) * 0.5f);  // 各推一半
    _positions[a] -= correction;
    _positions[b] += correction;
...
到这里，CPU 求解器的一个子步长成了完整形态。这是前六篇所有内容的汇总——**记住这个顺序，它就是软体求解的主线**：
...
private void Step(float deltaTime, int substeps, SoftBodyStepParameters parameters)
{
    ...
    _isGrounded = false;
    Array.Clear(_contactNormals, 0, _contactNormals.Length);

    BeginStep();                                                    // 01 快照
    ApplyExternalForces(deltaTime, parameters.Gravity, ...);        // 01 重力→速度
    ApplySpringForces(deltaTime, ...);                              // 02 弹簧→速度
    Integrate(deltaTime);                                           // 01 速度→位置
    ProjectShapeMatching(deltaTime, ..., wasGrounded);              // 03 形状记忆
    if (!wasGrounded)
        ProjectRadialVolume(deltaTime, ...);                        // 04 空中体积
    ProjectSurfaceWelds();

    if (parameters.EnableSelfCollision)
    {
        SolveSelfCollision(...);                                    // 06 自碰撞
        ProjectSurfaceWelds();
    }
    SolveUnityColliders(...);                                       // 05 任意 Collider
    if (parameters.UseGroundPlane)
        SolveGroundCollision(parameters.GroundHeight);              // 05 地面
    if (_isGrounded)
        ProjectGroundedVolume(deltaTime, ...);                      // 04 落地体积
    ProjectSurfaceWelds();

    UpdateVelocities(deltaTime, substeps, ...);                     // 01 反推速度+接触响应
}
```

> [!tip] 顺序不是随意的
> 力（重力+弹簧）在积分前；所有位置投影（形状/体积/碰撞/自碰撞）在积分后；`UpdateVelocities` 收尾把位置变化反推回速度。这个「力 → 积分 → 一串位置投影 → 反推速度」正是 [[07 PBD 与 PBF]] 要讲的 PBD 循环——下一篇你会看到它的理论骨架。
