...
朝向和缩放直接跟质点会抖（质点每帧有微小波动）。用[[01 质点系统与时间积分]]见过的**帧率无关指数平滑** `1 - exp(-k·dt)`：
...
> `1 - exp(-k·dt)` 在 [[01 质点系统与时间积分]] 是位置投影比率、在 [[03 形状匹配：整体记忆]] 是形状恢复比率、在这里是表情平滑比率——**同一个「帧率无关指数趋近」公式，贯穿整个项目**。学透一次，处处能用。
...
- 在 `LateUpdate` 摆位（物理更新后），用 `1-exp(-k·dt)` 平滑朝向（Slerp）和缩放（Lerp）。
