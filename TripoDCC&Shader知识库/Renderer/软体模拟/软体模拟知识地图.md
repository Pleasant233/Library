...
> 我们要在 Unity 里，从一个空工程开始，一步步做出一只完整的史莱姆——会压扁、会回弹、能被推着走、表面是半透明果冻、还有一张会眨眼的脸。
> 这不是一堆零散的踩坑记录，而是一条**手把手的路径**：每篇都在上一篇能跑的工程上，再填一层，讲清「为什么要这层 / 它解决什么问题 / 代码怎么长出来 / 我踩过什么坑」。跟着做，你的工程会和文字同步长大。
> 参照 [[前端知识地图]] 的分层：**入门（建心智 + 搭工程）→ 原理（一层约束一篇）→ 进阶（并行与工程）→ 专项（渲染与表情）**。
> **从 [[00 什么是软体模拟]] 开始，别跳。**
...
- 工程源码：`https://github.com/Pleasant233/Graphics-Learning`
...
| 入门 | [[00.1 从零搭起：工程骨架]] | 工程怎么建、代码住在哪、每帧怎么被调用 |
...
> [!tip] 怎么用这个知识库
> - **想动手做**：00.1 搭好工程骨架 → 01 → 06 顺着写，每篇的代码都能直接跑，最后拼成完整求解器。
