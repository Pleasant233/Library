...
> 承接 [[02 弹簧约束：局部弹性]]。上一篇结尾我们发现一个头疼的问题：光靠弹簧，史莱姆揉几下就塌成一摊、回不来原形——「布袋化」。这一篇就来解决它：给史莱姆装上「形状记忆」。
...
这一篇的新增是**第一个位置投影**——它插在 `Step()` 的 `Integrate()` **之后**（弹簧是力、发生在积分前；形状匹配是位置投影、发生在积分后）：
...
private void Step(float deltaTime, int substeps, SoftBodyStepParameters parameters)
{
    ...
    bool wasGrounded = _isGrounded;   // 落地状态决定是否关掉形状匹配
    BeginStep();
    ApplyExternalForces(deltaTime, parameters.Gravity);
    ApplySpringForces(deltaTime, parameters.SpringStiffness, parameters.SpringDamping);
    Integrate(deltaTime);
    ProjectShapeMatching(deltaTime, parameters.ShapeMatchingStrength,   // ← 新增
                         parameters.GroundedShapeMatching, wasGrounded);
    UpdateVelocities(deltaTime, substeps, parameters.Damping);
}

// 本阶段核心：把每个点往「当前质心 + 它的静止偏移」拉一点
private void ProjectShapeMatching(float deltaTime, float strength,
                                  float groundedStrength, bool isGrounded)
{
    Vector3 center = CalculateCenter(_positions);          // 当前质心
    float rate = isGrounded ? strength * groundedStrength : strength;  // 落地时可关掉
    float correction = ToProjectionFraction(rate, deltaTime);  // 帧率无关比率（见 01）

    for (int i = 0; i < _positions.Length; i++)
    {
        Vector3 target = center + _restOffsets[i];          // 目标 = 质心 + 原始偏移
        _positions[i] = Vector3.Lerp(_positions[i], target, correction);
    }
}
...
其中 `_restOffsets[i]`（每点相对静止质心的偏移）由 `SlimeTopology` 在初始化时算好：`offsets[i] = positions[i] - center`。
